...
class EfficientNet(nn.Module):
    ...
    def pretrained(cls, model_name, num_classes=1000, tuned_classes=2,in_channels = 3):
        ...
        load_pretrained_weights_Disk(model, model_name, load_fc=(num_classes == 2))
        if in_channels != 3:
            Conv2d = get_same_padding_conv2d(image_size = model._global_params.image_size)
            out_channels = round_filters(32, model._global_params)
            model._conv_stem = Conv2d(in_channels, out_channels, kernel_size=3, stride=2, bias=False)
    ...
    @classmethod
    def loadTrainedModel(cls, model, model_name):
        model.load_state_dict(torch.load("pretrainedModel/" + model_name + ".pth"))
